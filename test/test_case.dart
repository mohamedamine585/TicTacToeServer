...
test_case(List<String> playernames, List<String> passwords, String host1,
    String host2) {
  var message0;
  group('************** Test ***************', () {
    group('**********  Test Create***************', () {
    ...
    group('******* Test Delete  *******', () {
      for (int i = 0; i < 3; i++) {
        ...
        test('Delete ${i / 2} 1', () async {
          var response = await get(Uri.parse(
              '$host2/Delete/?playername=${playernames[i + 1]}&password=${passwords[i + 1]}'));
          expect(jsonDecode(response.body)["res"], true);
          expect(jsonDecode(response.body)["res"], true);
        });
        i++;
